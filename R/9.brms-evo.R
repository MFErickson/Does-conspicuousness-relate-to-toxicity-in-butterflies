...
dt_raw <- read.csv("output/conspicuousness.csv")

# Remove rows where Time.difference is NA
dt <- dt_raw %>%
  filter(!is.na(Time.difference))
...
# Save output
output_file <- "output/brms-exclude ventral only-result.txt"
sink(output_file)
